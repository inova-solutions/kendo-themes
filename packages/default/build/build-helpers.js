...
exports.getThemeFile = (theme, distPath) =>
    `${distPath}/inova-${theme.toLowerCase()}.css`;
        ...
        .map((line) => {
            if (line.indexOf(" max(") >= 0) {
                ...
                const interpolatedLine =
                    beforeMax + interpolatedMaxContent + afterMax;
...
exports.wrapWithThemePrefix = (theme, path) => {
    ...
    const finalCss =
        wrappedCss.slice(0, ffClassHeadStart) +
...
exports.combineThemes = (themes, path) => {
    ...
    while (this.occurrences(css, "@font-face") > 1) {
        ...
        css = css.slice(0, ffStart) + css.slice(ffEnd + 2);
    }
...
exports.compileInovaIcons = (source, output) => {
    ...
    let newSource = ffTemplate.replace(
        "ENTER_BASE64_FONT_FILE_HERE",
        fontBase64
    );
    ...
    css = css.slice(0, startFfSource) + newSource + css.slice(endFfSource + 2);
...
exports.createLegacyThemeOutput = (theme, path) => {
    ...
    switch (theme) {
        case "Light":
            legacyThemeDir = "OlivePink";
            break;
        case "Dark":
            legacyThemeDir = "DarkPink";
            break;
    }
...
exports.createPackageJson = (path) => {
    ...
    let currentVersion = this.exec(`npm show ${json.name} version`, true)
        .toString()
        .split(".");
...
exports.publishHelper = (path) => {
    ...
    prompt.get([question], function (err, result) {
        ...
        if (result[question].toLowerCase() === "y") {
            exec(`npm publish ${process.cwd() + "\\" + path}`);
        } else {
            writeLog(
                "To publish, run following command: \n\nnpm publish " +
                    process.cwd() +
                    "\\" +
                    path +
                    "\n"
